...
- **Unified per-type table.** Each type's changes render as a single table with columns `Path | Change | <old filename> | <new filename>`, sorted alphabetically by path (then by kind) so all changes for the same field cluster together. The change kinds emitted in the `Change` column: `field added`, `field removed`, `type`, `description`, `arg_added`, `arg_removed`, `arg_changed`, `enum added`, `enum removed`. Row-level left-border accents (`tr.added` / `tr.removed` / `tr.changed`) indicate category. Both HTML and Markdown flow through the same `_unified_rows(td)` helper in [jdiff/render.py](jdiff/render.py) so the two outputs stay in sync.
- **HTML change-kind filter.** Toolbar at the top of `<main>` with one checkbox per change kind — all checked by default ("Show X"). Unchecking a kind hides every matching row (tagged via `data-kind` on each `<tr>`) across every table in the report. Preset buttons: **Hide description** (unchecks only `description`, the common reviewed-once-now-noise case) and **Show all** (re-checks everything). State persists in `localStorage` under key `jdiff-shown-kinds-v2`. Vanilla JS, no deps. Markdown output has no filter — Confluence renders no JS, and the unfiltered table is the intended paste artefact.
...
- **2026-04-15** — responded to first round of review feedback on the initial report. Two changes: (1) collapsed the five per-type sub-tables (Fields added / removed / changed, Enum added / removed) into a single unified table with columns `Path | Change | <old> | <new>`, sorted alphabetically by path so all changes for the same field cluster; row-level left-border accents preserve the green/red/blue change-category signal. Flattening helper `_unified_rows` in [jdiff/render.py](jdiff/render.py) drives both HTML and Markdown so the two outputs stay in sync. (2) Added an HTML change-kind filter toolbar at the top of `<main>` — one checkbox per change kind, all **checked by default** (label "Show"), unticking hides every row of that kind via a `data-kind` attribute on each `<tr>`. Preset buttons: **Hide description** (the reviewed-once-then-noise case the reviewer called out) and **Show all**. State persists in `localStorage` (`jdiff-shown-kinds-v2`). Vanilla JS, no deps. Toolbar styled as a flex row with a subtle separator between checkboxes and buttons; sticky ToC is unaffected. Added [tests/test_render.py](tests/test_render.py) covering the flattener and the toolbar markup.
