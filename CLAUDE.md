...
- **2026-04-15** — collapsed per-type sub-tables (Fields added/removed/changed, Enum added/removed) into a single unified table per type with columns `Path | Change | <old> | <new>`, sorted alphabetically by path so all changes for the same field cluster. Added an HTML filter toolbar with one checkbox per change kind plus a "Hide description only" preset; state persists in `localStorage`. Flattening helper `_unified_rows` in [jdiff/render.py](jdiff/render.py) drives both the HTML template and the Markdown output so they stay in sync. Markdown has no filter (Confluence renders no JS).
